...
function CLBP_SMCjoint = clbpJoint(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.2
...
CLBP_SMCjoint = CLBP_C*bins*bins+CLBP_S*bins+CLBP_M;

if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    % Return with LBP histogram if mode equals 'hist'.
%     CLBP_S=hist(CLBP_S(:),0:(bins-1));
%     CLBP_M=hist(CLBP_M(:),0:(bins-1));
    CLBP_SMCjoint=hist(CLBP_SMCjoint(:),0:(bins*bins*2-1));

    if (strcmp(mode,'nh'))
%         CLBP_S=CLBP_S/sum(CLBP_S);
%         CLBP_M=CLBP_M/sum(CLBP_M);
        CLBP_SMCjoint=CLBP_SMCjoint/sum(CLBP_SMCjoint);
